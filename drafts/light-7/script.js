...
(function drawQR() {
  ...
  const finder = (cx, cy) => {
    x.fillStyle = '#241f15'; x.fillRect(off + cx * m, off + cy * m, 7 * m, 7 * m);
    x.fillStyle = '#ffffff'; x.fillRect(off + (cx + 1) * m, off + (cy + 1) * m, 5 * m, 5 * m);
    x.fillStyle = '#241f15'; x.fillRect(off + (cx + 2) * m, off + (cy + 2) * m, 3 * m, 3 * m);
  };
...
(function buildStack() {
  ...
  const inks = ['#2f5b7e', '#e27820', '#a8843e', '#ddd5bd', '#45423a'];
